...
class Users {
  ...
  const { email, password, fcm, token, type, role, user_name , created_by } = req.body;
  const image = req.file; // 👈 yeh multer se aayega
  ...
  try {
    ...
    const imagePath = image ? image.path : null; // 👈 Save image path

    const [result] = await pool.query(
      `INSERT INTO users (user_name, email, password, fcm, token, type, email_verified_status, status, role, is_delete, image , created_by)
       VALUES (?, ?, ?, ?, ?, ?, 1, 1, ?, 'false', ?,?)`,
      [user_name, email, hashedPass, fcm, token, type, role, imagePath , created_by]
    );
...
async login(req, res) {
  ...
  try {
    ...
    if (type === "EMAIL") {
      ...
      if (!existingUser.password) {
        return res.status(400).json({ error: true, msg: "No password set for this user" });
      }
    ...
    if (fcm) {
      await pool.query("UPDATE users SET fcm = ? WHERE id = ?", [fcm, existingUser.id]);
    }
  ...
  try {
    ...
    const updatableFields = [
      "full_name", "user_name", "country", "city", "phone_no", "location_address",
      "location_log", "location_lat", "email", "stripe_subscription_status",
      "email_verified_status", "fcm", "status", "role", "live_image", "cnic",
      "subscription", "phone_no_verified_status", "email_code", "created_at",
      "verify_status", "bitcoin", "paypal", "bank", "type", "created_by",
      "reset_password_otp", "password", "token", "is_delete"
    ];
    ...
    if (fields.length === 0) {
      return res.status(400).json({ error: true, msg: "No fields provided for update" });
    }
 ...
 async getCustomers(req, res) {
  ...
  try {
    ...
    if (rows.length === 0) {
      return res.status(404).json({ error: true, msg: "No customers found for this user" });
    }
...
async changePassword(req, res) {
  ...
  if (!oldPassword || !newPassword) {
    return res.status(400).json({ error: true, msg: "Old and new passwords are required" });
  }
  ...
  try {
    ...
    return res.status(500).json({ error: true, msg: "Internal server error", technicalIssue: error.message });
  }
}
...
async forgotPassword(req, res) {
  ...
  try {
    ...
    // Save OTP in DB
    await pool.query("UPDATE users SET reset_password_otp = ? WHERE email = ?", [otp, email]);
...
async verifyOtp(req, res) {
  ...
  try {
    const [users] = await pool.query("SELECT * FROM users WHERE email = ? AND reset_password_otp = ?", [email, otp]);

    if (users.length === 0) {
      return res.status(400).json({ error: true, msg: "Invalid OTP or email" });
    }

    return res.status(200).json({ error: false, msg: "OTP verified successfully" });
  } catch (error) {
...
async resetPassword(req, res) {
  ...
  try {
    ...
    await pool.query("UPDATE users SET password = ?, reset_password_otp = NULL WHERE email = ?", [hashedNewPassword, email]);

    return res.status(200).json({ error: false, msg: "Password reset successfully" });
  } catch (error) {
